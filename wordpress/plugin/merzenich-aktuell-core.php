...
define('MA_CORE_VERSION', '1.1.0-recovery');
...
register_activation_hook(__FILE__, function () {
    ...
    foreach (['Merzenich'=>'merzenich','Golzheim'=>'golzheim','Girbelsrath'=>'girbelsrath','Morschenich'=>'morschenich','Bürgewald'=>'buergewald'] as $name=>$slug) { if (!term_exists($slug,'ma_location')) wp_insert_term($name,'ma_location',['slug'=>$slug]); }
    foreach (['Neu','Prüfen','Übernommen','Ignoriert','Duplikat'] as $status) { if (!term_exists($status,'ma_source_status')) wp_insert_term($status,'ma_source_status'); }
