...
(()=>{
 ...
 fetch('/api/sport-current.json',{cache:'no-store'}).then(r=>{if(!r.ok)throw new Error('sport '+r.status);return r.json();}).then(data=>{
  // Serverseitig schon aus genau diesem Datenstand gerendert: nichts umbauen.
  if(module.dataset.generated&&module.dataset.generated===data.generated)return;
